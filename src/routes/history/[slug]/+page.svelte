...
<div class="form-page">
	<section class="wiki-form">
		...
		{#if data.selected}<h2>리비전 {data.selected} 변경 사항</h2>
			<div class="diff-box">
		...
		<ul class="history-list">
			{#each data.revisions as revision}<li>
					...
					>
					{#if revision.summary?.startsWith('[AI 통합 · 새 초안 우선]')}
						<details class="merge-history">
							<summary><span class="tag">AI 통합</span> 새 초안 우선 · 상충 내용 확인</summary>
							<p>{revision.summary}</p>
						</details>
					{:else}<span>{revision.summary || '요약 없음'}</span>{/if}
					<span
						><a href={`?diff=${revision.id}`}>차이 보기</a> ·
						<form method="POST" action="?/rollback" style="display:inline">
							<input type="hidden" name="revision" value={revision.id} /><button
								class="danger-button">이 버전으로 되돌리기</button
							>
						</form></span
					>
				</li>{/each}
