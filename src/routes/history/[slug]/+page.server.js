...
export async function load({ params, url }) {
	...
	let changes = [];
	if (selected) {
		const index = revisions.findIndex((item) => Number(item.id) === selected);
		const oldText = revisions[index + 1]?.content || '';
		changes = diffLines(oldText, revisions[index]?.content || '');
	}
...
export const actions = {
	rollback: async ({ request, params }) => {
		...
		const [revision] =
			await db()`SELECT content FROM revisions WHERE id=${id} AND document_id=${result.document.id}`;
		...
		await saveDocument({
			...
			originalSlug: result.document.slug
		});
