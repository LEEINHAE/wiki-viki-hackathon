	...
	<section class="wiki-form">
		...
		<nav class="document-actions">
			<a href={`/wiki/${data.document.slug}`}>읽기</a><a href={`/edit/${data.document.slug}`}
				>편집</a
			><a href={`/history/${data.document.slug}`}>역사</a>
		</nav>
		{#if form?.message}<div class="notice warning">{form.message}</div>{/if}{#if form?.success}<div
				class="notice"
			>
				토론을 등록했습니다.
			</div>{/if}
		{#each data.threads as thread}<article class="thread">
				<div class="thread-head">
					<b>{thread.thread_title}</b><span
				...
				아직 토론이 없습니다. 아래에서 첫 토론을 시작해 보세요.
			</p>{/each}
		<h2>새 토론 시작</h2>
		<form method="POST">
			<div class="field">
			...
			<div class="button-row"><button class="primary-button">토론 등록</button></div>
		</form>
	</section>
</div>
