...
export async function load({ params }) {
	try {
		...
		return { slug: params.slug, document: null, databaseError: error.message };
	}
}
export const actions = {
	save: async ({ request, params }) => {
		...
		if (!governance.passed)
			return fail(400, {
				message: `콘텐츠 보호 검사로 저장이 차단되었습니다: ${governance.reasons.join(', ')}`,
				title,
				content,
				editor,
				summary,
		...
		try {
			const document = await saveDocument({
				...
				editor,
				summary,
				originalSlug: (await getDocument(params.slug))?.document.slug || slugify(title)
			});
			for (const alias of aliases
				.split(',')
				.map((value) => value.trim())
				.filter(Boolean))
				await db()`INSERT INTO redirects (alias_slug,alias_title,document_id) VALUES (${slugify(alias)},${alias},${document.id}) ON CONFLICT (alias_slug) DO UPDATE SET alias_title=EXCLUDED.alias_title,document_id=EXCLUDED.document_id`;
			redirect(303, `/wiki/${encodeURIComponent(document.slug)}`);
		} catch (error) {
			if (isRedirect(error)) throw error;
			return fail(500, { message: error.message, title, content, editor, summary, aliases });
		}
	},
	delete: async ({ params }) => {
		const result = await getDocument(params.slug);
