...
<main class="form-page">
	...
	<form class="wiki-form" method="POST" action="?/save">
		...
		<div class="field">
			<label for="content">위키 본문</label><textarea id="content" name="content" required
				>{form?.content ??
					...
					'## 개요\n\n이곳에 문서 내용을 작성하세요. [[문서 이름]] 형식으로 다른 문서를 연결할 수 있습니다.'}</textarea
			><small>Markdown, [[위키 링크]], 표, 각주, 인용문, 코드를 지원합니다.</small>
		</div>
		<div class="field">
		...
		<div class="form-row">
			<div class="field">
				<label for="editor">익명 편집자 이름</label><input
					id="editor"
					name="editor"
	...
	{#if data.document}<form
			...
			action="?/delete"
			onsubmit={(event) => {
				if (!confirm('문서와 모든 이력, 토론을 삭제할까요? 이 작업은 되돌릴 수 없습니다.'))
					event.preventDefault();
			}}
		>
			<div class="button-row"><button class="danger-button">문서 삭제</button></div>
		</form>{/if}
</main>
