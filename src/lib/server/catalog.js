...
export async function getCatalog({ drafts = false } = {}) {
	...
	const [documents, redirects, pending] = await Promise.all([
		sql`SELECT * FROM wv_visible_documents WHERE deleted_at IS NULL ORDER BY updated_at DESC`,
		sql`SELECT alias_title, alias_slug, document_id FROM redirects r WHERE EXISTS (SELECT 1 FROM wv_visible_documents d WHERE d.id=r.document_id AND d.deleted_at IS NULL)`,
		drafts
			? sql`SELECT * FROM wv_visible_drafts WHERE status <> 'published' ORDER BY created_at DESC`
			: []
	]);
