-- EXPLAIN on 1,000 synthetic documents showed repeated normalization cost.
-- Compute normalized text only when title/content change, without another search service.
ALTER TABLE documents ADD COLUMN IF NOT EXISTS wv_search_text TEXT GENERATED ALWAYS AS (wv_normalize_v1(title||' '||content)) STORED;
